...
#ifndef _OFX_LIMITED_POINT_H
#define _OFX_LIMITED_POINT_H
...
public:
	...
	ofxLimitedPoint( const ofPoint & pnt)
		...
		minLimit.set(FLT_MIN,FLT_MIN,FLT_MIN);
	...
	float getLimitedY() {return CLAMP(x,minLimit.x,maxLimit.x);}
	float getLimitedZ() {return CLAMP(x,minLimit.x,maxLimit.x);}
	...
	void clearMinLimits() { minLimit.set(FLT_MIN,FLT_MIN,FLT_MIN); onLimitChange();}
